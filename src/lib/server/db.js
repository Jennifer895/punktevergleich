...
export function endWeekAndReset({ pointsA, pointsB, netDifference}) {
    ...
    db.prepare('INSERT INTO history (date, pointsA, pointsB, netDifference, overallNetScore) VALUES (?, ?, ?, ?, ?)')
      .run(resetTimestamp, pointsA, pointsB, netDifference, newOverallScore);
    
    db.prepare('UPDATE current SET scoreA = 0, scoreB = 0, lastResetDate = ? WHERE id = 1').run(resetTimestamp);
}
...
export function checkAndRunWeeklyReset(){

    const stand = getCurrentScores();
    if (!stand) return;

    const lastResetDate = new Date(stand.lastResetDate);
    const now = new Date();
   ...
   targetReset.setDate(now.getDate() - daysSinceMonday);
   targetReset.setHours(0,0,0,0);

   if ( lastResetDate < targetReset){

    console.log("Entscheider: Zeit für automatischen Reset!");
    const netDifference = stand.scoreA - stand.scoreB;
    ...
    console.log("Entscheider: Woche läuft noch. Kein Reset nötig.");
   }
